...
with strategy.scope():
    multi_worker_dataset = strategy.distribute_datasets_from_function(
        lambda input_context: dataset.dataset_fn(global_batch_size, input_context))
    model = dataset.build_cnn_model()
    ...
    checkpoint= tf.train.Checkpoint(model= model, epoch= epoch, step_in_epoch= step_in_epoch)
    ...
    while epoch.numpy() < EPOCHS: 
        ...
        print('Epoch: %d, accuracy: %f, training_loss: %f'%(epoch.numpy(), train_accuracy.result(), train_loss))
