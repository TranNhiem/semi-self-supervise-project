...
class Conv_stage1_block(tf.keras.Model):
    def __init__(self, filters, strides=2, mode="2D", norm="BatchNorm", kernel_initializer='he_normal', name=None):
        ...
        if mode == "1D" or mode == "2D" or mode == "3D":
            ...
            self.pool1 = TimeDistributed(MaxPooling(
    ...
    def __init__(self, filters, kernel_size=3, mode="2D", norm="BatchNorm", kernel_initializer='he_normal', name=None):
        ...
        if mode == "1D" or mode == "2D" or mode == "3D":
            ...
            self.conv1 = ConvLayer(
                filters1, 1, kernel_initializer=kernel_initializer, padding='same')
            self.bn2 = NormLayer()
            ...
            self.conv3 = ConvLayer(
                filters3, 1, kernel_initializer=kernel_initializer, padding='same')
        elif mode == "TimeD":
            self.bn1 = TimeDistributed(NormLayer())
...
class Conv_bottleneck_block(tf.keras.Model):
    def __init__(self, filters, kernel_size=3, strides=2, mode="2D", norm="BatchNorm", kernel_initializer='he_normal', name=None):
        ...
        if mode == "1D" or mode == "2D" or mode == "3D":
            ...
            self.relu1 = Activation('relu')
            self.conv1 = ConvLayer(
                filters1, 1, strides=strides, kernel_initializer=kernel_initializer, padding='same')
            self.bn2 = NormLayer()
            self.relu2 = Activation('relu')
            self.conv2 = ConvLayer(
                filters2,  kernel_size, kernel_initializer=kernel_initializer, padding='same')
            ...
            self.s_conv = TimeDistributed(ConvLayer(
                filters3, (1, 1), strides=strides, kernel_initializer=kernel_initializer, padding='same'))
...
class Conv_basic_block(tf.keras.Model):
    def __init__(self, filters, kernel_size=3, strides=2, mode="2D", norm="BatchNorm", kernel_initializer='he_normal', name=None):
        ...
        if mode == "1D" or mode == "2D" or mode == "3D":
            ...
            # Right(shortcut)
            self.s_bn = NormLayer()
            self.s_conv = ConvLayer(
                filters2, 1, strides=strides, kernel_initializer=kernel_initializer, padding='same')
        elif mode == "TimeD":
            # Left
            self.bn1 = TimeDistributed(NormLayer())
            self.relu1 = TimeDistributed(Activation('relu'))
...
class BuildResnet(tf.keras.Model):
    def __init__(self, class_num=1000, include_top=True, pooling='ave', mode="2D", width_scale=1, norm="BatchNorm", kernel_initializer='he_normal', name=None):
        ...
        self.stage_filter = [
            filter_*self.width_scale for filter_ in filters_stage]
        print(
            f"your width_scale {  self.width_scale} & filter stage { self.stage_filter}")
        ...
        # stage1
        self.conv1 = Conv_stage1_block(
            filters=all_filters[0][0], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        # stage2
        self.stage2_convs = {}
        self.stage2_convs[0] = ConvBlock(
            filters=all_filters[0], strides=1, mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        for rep in range(1, self.reptitions[0]):
            self.stage2_convs[rep] = IdBlock(
                filters=all_filters[0], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        # stage3
        self.stage3_convs = {}
        self.stage3_convs[0] = ConvBlock(
            filters=all_filters[1], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        for rep in range(1, self.reptitions[1]):
            self.stage3_convs[rep] = IdBlock(
                filters=all_filters[1], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        # stage4
        self.stage4_convs = {}
        self.stage4_convs[0] = ConvBlock(
            filters=all_filters[2], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        for rep in range(1, self.reptitions[2]):
            self.stage4_convs[rep] = IdBlock(
                filters=all_filters[2], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        # stage5
        self.stage5_convs = {}
        self.stage5_convs[0] = ConvBlock(
            filters=all_filters[3], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        for rep in range(1, self.reptitions[3]):
            self.stage5_convs[rep] = IdBlock(
                filters=all_filters[3], mode=mode, norm=norm, kernel_initializer=kernel_initializer)
        # Final Layer
        if self.pooling != None:
            self.fin = Fin_layer(
                mode=mode, include_top=include_top, class_num=class_num, pooling=self.pooling)
